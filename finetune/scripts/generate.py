...
from Generation.sampling_strategies import SamplingConfig, get_strategy, PRESETS, DEFAULT_STRATEGY
...
class Trainer:
    ...
    def evaluate_generate(self, text, idx, out_dir, agent, env,
                          sampling_config=None, use_viewer=False, **kargs):
        ...
        # OpenGL 3D viewer (optional)
        viewer = None
        if use_viewer:
            from Visualization.ode_viewer import ODEViewer
            viewer = ODEViewer(agent.env)
            viewer.start()
        ...
        try:
            from Visualization.multi_angle_viewer import visualize_multi_angle
